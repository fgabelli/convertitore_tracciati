...
# Chiave segreta Stripe e secret del webhook
stripe.api_key = os.getenv("STRIPE_API_KEY")
endpoint_secret = os.getenv("STRIPE_ENDPOINT_SECRET")

# File credenziali e profili
CREDENTIALS_FILE = "user_credentials.json"
PROFILE_DIR = "profiles"
os.makedirs(PROFILE_DIR, exist_ok=True)

# Flask per la gestione dei webhook Stripe
app = Flask(__name__)

# Carica e salva credenziali utente
...
def stripe_webhook():
    ...
    # Evento per completamento pagamento
    if event["type"] == "checkout.session.completed":
        ...
        for username, user_data in credentials["usernames"].items():
            if user_data["email"] == customer_email:
                ...
                print(f"Utente {username} è ora Premium.")
...
# Avvia il server Flask per i webhook
def start_flask():
    app.run(port=5000)
